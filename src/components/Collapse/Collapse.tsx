...
export const Collapse = forwardRef<HTMLDivElement, CollapseProps>(
    ({ items, activeKeys: controlledKeys, defaultActiveKeys = [], accordion = false, onChange, className }, ref) => {
        ...
        return (
            <div ref={ref} className={[styles.collapse, className].filter(Boolean).join(' ')}>
                {items.map((item) => {
                    const isActive = activeKeys.includes(item.key);
                    const bodyId = `collapse-panel-${item.key}`;
                    return (
                        <div
                            key={item.key}
                            className={[
